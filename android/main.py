...
class HeartbeatApp(App):
    ...
    def on_start(self):
        """Called when app starts - auto-start heartbeat if PC already discovered"""
        print("App started")

    def on_pause(self):
        """Called when app goes to background"""
        print("App paused - keeping heartbeat alive")
        return True  # Keep app running in background

    def on_resume(self):
        """Called when app resumes from background"""
        print("App resumed")
        # Check if heartbeat needs restart
    ...
    def update_discovered_ip(self, pc_ip, heartbeat_port):
        ...
        # Auto-start heartbeat if not already sending
        if not self.sending:
